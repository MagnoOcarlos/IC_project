...
do
	...
	mkdir -p ../results/many_sphere/${i}_Mpc
	
	#loop for the angles and to run the geometry code to each one
	for((j=0; j<=80; j=j+20))
	do
		cp *radius_${i}_Mpc_many_* aux.txt
		...
		python  geometry.py aux.txt $j ../results/many_sphere/${i}_Mpc/results_ang$j.txt
	done
	
	cd ../results/many_sphere/${i}_Mpc
