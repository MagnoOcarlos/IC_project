...
#################################################################
	...
	#
	#To run this file look the bashs' scripts in the geometry directory	
...
do
	cat $path/results_R$R.txt | awk '{print $5}'>E.txt
	cat $path/results_R$R.txt | awk '{print $14}'>E0.txt
	
	sed -i -e '1d' E*

	cp E.txt ${R}Mpc.txt 

	paste E0.txt  E.txt | awk '{ delta=($1-$2)/($1) ; printf"%0.9f\n", delta}' >$path/delta_${R}Mpc.txt

	rm E*
	
	cat $path/results_R$R.txt | awk '{print $1}'>$path/distance_${R}Mpc.txt
	
	sed -i -e '1d' $path/distance_${R}Mpc.txt
done
